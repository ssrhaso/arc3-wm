...
> in `CLAUDE.md` Section "Phases". Each phase is locked behind these assertions -
...
|---|---|---|
| Repo scaffold | `pyproject.toml`, `.gitignore`, `.env.example`, dirs from CLAUDE.md Section "Repo layout" all present | [x] |
| `arc_agi` import | `python -c "import arc_agi; importlib.metadata.version('arc-agi')"` prints `0.9.8` | [x] (note: `arc_agi.__version__` does NOT exist - use `importlib.metadata`) |
| `.env` loaded | `OPERATION_MODE=offline` + `ARC_API_KEY=...` in `.env`; `arc_agi.base` auto-loads via `python-dotenv` | [x] |
| OFFLINE active | `arc_agi.Arcade().operation_mode == OperationMode.OFFLINE` after import | [x] (verified) |
| Env files cached | `environment_files/{vc33,tu93,cd82}/<version>/{metadata.json,<game>.py}` exist | [x] (3 games cached for the smoke; full 25 needed before Phase 3) |
| Docs fetched | All pages listed in Section 6 of the Phase-0 instructions present under `docs/arc-agi-3/` | [x] |
| `harness-analysis.md` written | Covers obs/action/reward/scorecard/modes/list_actions; answers RHAE question | [x] |
| `replay-format.md` written | Per-line schema, action/reward extraction, worked example | [x] |
| `compute-runbook.md` written | Vast.ai launch + resume + data staging | [x] |
| `phase-checklists.md` written | This file | [x] |
| References cloned | `third_party/{ARC-AGI-3-Agents,dreamerv3}/` populated | [x] |
| Replays fetched | `find data/replays -name '*.jsonl' \| wc -l == 342` | [fail] **39/342 - Drive quota blocker.** See `docs/replay-format.md` Section "Open issue" |
| Random-agent smoke | `scripts/random_agent_smoke.py` runs 10 episodes on vc33 OFFLINE, no exceptions, scorecard populated | [x] (921 FPS, score=0 with random play, expected) |
| RHAE source-of-truth | Documented whether `get_scorecard()` returns RHAE | [x] - yes, on 0-100 scale |

**Phase 0 exit gate:** all rows above [x] except the replay count, plus
...
| `tests/test_action_space.py` | `flat_to_arc(idx)` and `arc_to_flat(action_id, x, y)` round-trip identity for all `idx in [0, 4101]`; `flat_to_arc(0..4) -> ACTION1..ACTION5`; `flat_to_arc(5 + 64*y + x) -> (ACTION6, x, y)`; `flat_to_arc(4101) -> ACTION7`; `build_mask(env)` produces a length-4102 bool tensor whose `True` entries match the union over (action_id, x, y) tuples valid for the current `available_actions` (ACTION6 contributes 4096 indices when 6 in available, otherwise 0) | `arc3_wm/action_space.py` |
| `tests/test_reward.py` | Reward signal matches `delta levels_completed` between consecutive frames; on terminal `WIN` and `GAME_OVER` the wrapper sets `terminated=True` (not `truncated`); `truncated=True` only on `max_steps` timeout | `arc3_wm/env.py` |
...
| `tests/test_replay_loader.py` | Parses **all 342 JSONLs** (no sample); accepts `action_input.id` as int OR string; tolerates the trailing session-summary line; emits `(o_t, a_t, r_t, terminated_t, o_{t+1})` tuples with `o.shape == (64, 64, 3)`, `o.dtype == uint8`, `a in [0, 4101]`, `r in {-1, 0, 1, ...}` (just `delta levels_completed` unless Haso adds shaping); episode-boundary detection inside a single JSONL via `state in {WIN, GAME_OVER}` and trailing summary; total transition count > 0 and reasonable (per CLAUDE.md, ~30k-150k aggregate) | `arc3_wm/replay_loader.py` |
...
- Full action-space enumeration: every flat idx 0-4101 produces a
  ...
  and ACTION7 is correctly masked out on games where `7 not in available_actions`.
...
- **Layer-selection policy** for `fd.frame` (multi-layer animation frames).
  ...
  See `docs/harness-analysis.md` Section "Observations".
- **Reward shaping.** Current default: `r = delta levels_completed`.
...
**Where:** Vast.ai 1x H100 80GB spot. **~6-12 h.**
...
| Within 10% of reference | At 1M env steps, `score / 11.7 in [0.9, 1.1]` (Crafter reference reward; cite DreamerV3 paper Fig 4) |
...
**Where:** Vast.ai 1x H100 80GB spot. **~6 h.**
...
| Loss decreases monotonically | All four WM losses (recon, dynamics, reward, continue) trend down across 1 epoch; smoothed loss at end < 0.9 x smoothed loss at start (per loss) |
...
| Three games x two seeds | All 6 runs launch and reach 500k env steps without crashes |
...
| Check | Pass condition |
|---|---|
| All 25 x 3 seeds x 1M steps complete | 75 runs all reach the step budget; failed-run replacements documented |
...
| Check | Pass condition |
|---|---|
| Linear probes on RSSM latents | For each of {object position, last action, level index}, a logistic / linear probe on the trained-RSSM stochastic latent achieves accuracy >= random + 5sigma |
