...
> Framing decisions here are Haso's (CLAUDE.md Section "Decisions Haso owns"
...
controlled baseline: stock DreamerV3 on 6 public games x 2 seeds,
...
2. **Controlled negative result.** Stock DreamerV3 `size12m` (the
   ARC-1-precedented config, Lee et al. 2024), 6 games x 2 seeds x
   paired {cold, warm}. Cross-game offline WM pretraining gives delta
...
|---|---|---|---|
| C1 | First Gym/`embodied` wrapper for ARC-AGI-3 | `arc3_wm/{env,embodied_env}.py`; Living Survey "zero WM on ARC-3" | [x] shipped |
| C2 | Faithful action/obs/reward mapping | `tests/test_action_space.py` (round-trip 0-4101), `test_wrapper_spec.py`, `test_reward.py` | [x] green |
| C3 | Offline replay loader parses the full human dataset | `tests/test_replay_loader.py` over all 340 JSONLs | [x] green |
| C4 | RHAE implemented to methodology (D-A/D-B) | `arc3_wm/rhae.py`; `tests/test_rhae.py` (50); coverage 70.49% | [x] green |
| C5 | Cross-game WM pretraining converges | Phase-3 ckpt; WM losses down monotone | [x] done |
...
| C7 | Cross-game pretraining gives no measurable lift (paired) | warm-cold delta vs seed variance; `analysis/p4_fromscratch_vs_proper.md` | [~] pilot in; expansion running |
| C8 | WM fits while controller fails (the diagnosis) | train losses (image 53->0.14, dyn down) vs RHAE~0; Phase-6 probes/FVD/axis | [ ] Phase 6 |

## The 6-game benchmark

Pilot {vc33, sb26, cd82} + expansion {tn36, ls20, lf52}. Expansion
chosen for RHAE coverage + a human-hardness spread (fun/hard ratings x
...
- N = 6 games x 2 seeds - modest; the diagnosis (C8) carries the
...
- [x] **License** - MIT (`LICENSE`; `pyproject` `license` field +
  classifier set; README updated). (c) 2026 Hasaan Ahmad.
