...
```bash
pytest -q tests/test_rhae.py                 # → 33 passed
pytest -q tests/test_compute_rhae.py         # → 19 passed
pytest -q tests/test_extract_human_baselines.py  # → 29 passed
```
...
**Known gap**: DV3's eval logfn at
`third_party/dreamerv3/embodied/run/train_eval.py:67` pops the
per-step `rewards` stack before adding to `epstats`, so the stream
is NOT preserved in `scope/metrics.jsonl` or wandb today.
`compute_rhae.py` will run cleanly against a synthetic episodes
file (and the CLI's tests cover that path), but real-data
end-to-end requires either a small launcher-side sink that records
per-eval-episode rewards as it observes them, or preserving the
stack through `epstats`. Capturing the stream is a separate change
out of scope for this dry-run; the dry-run's load-bearing gate
("level 1 cleared on vc33?") is answered directly from
`eval/episode/score`.
