...
The JSONL schema, and design decisions D4 and D5 for reward and
id-parsing, are documented in the module functions below.
    ...
    for line_no, obj in _parse_lines(path):
        ...
        if pending and pending[-1][1].get("state") in TERMINAL_STATES:
            ...
            raise ReplayParseError(
                ...
                f"for review per design decision D5)"
            )
        ...
        if pending and not is_explicit_reset:
            ...
            if curr_levels < prev_levels:
                ...
                raise ReplayParseError(
                    ...
                    f"design decision D5 for the post-terminal-noise rule"
                )
