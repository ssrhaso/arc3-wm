...
Milestone (1) of the Phase-1 rescoping:
...
class ARC3GymEnv(gym.Env):
    ...
    def __init__(
        ...
        """``full_reset=True`` makes every ``reset()`` start a fresh game at
        level 1 by re-running ``Arcade.make``. The engine's own RESET only
        restarts the *current* level once a level has been cleared and the
        game has ended (verified on vc33, 2026-09-05: after a level-1 clear
        and a GAME_OVER on level 2, RESET returns the level-2 start frame
        with ``levels_completed == 1``), so with the default ``False`` an env
        instance offers level 1 only until it clears it once. Evaluation that
        means "attempt the game from level 1" must use ``full_reset=True``."""
        super().__init__()
        ...
        self._full_reset = bool(full_reset)
    ...
    def __repr__(self) -> str:
        ...
        return (
            ...
            f"full_reset={self._full_reset}, render_mode={self.render_mode!r})"
        )
    ...
    def reset(
        ...
        *,
        ...
        if self._full_reset or (seed is not None and seed != self._seed):
            # arc_agi seed is set at make() time; rebuild the wrapper to honour
            # it. Under full_reset the wrapper is rebuilt on every reset so the
            # episode starts at level 1 regardless of what was cleared before.
            if seed is not None:
                self._seed = int(seed)
