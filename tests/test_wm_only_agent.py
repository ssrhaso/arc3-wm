...
_STACK_PREFIXES = ("jax", "portal", "dreamerv3", "embodied")
...
def test_module_imports_without_jax():
    ...
    The evicted ``sys.modules`` entries are restored afterwards. Leaving
    them out is not harmless where JAX is installed: re-importing ``jax``
    after a partial eviction trips a circular import ("partially
    initialized module 'jax' has no attribute 'version'"), which made
    every later JAX-only test in this process fail on HPC (2026-09-05).
    """
    evicted = {k: sys.modules.pop(k) for k in list(sys.modules)
               if k.startswith(_STACK_PREFIXES)}
    try:
        ...
        for forbidden in _STACK_PREFIXES:
            assert forbidden not in sys.modules, (
                f"top-level import of {forbidden!r} leaked into wm_only_agent"
            )
    finally:
        for k in [k for k in list(sys.modules) if k.startswith(_STACK_PREFIXES)]:
