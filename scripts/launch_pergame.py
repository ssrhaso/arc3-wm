...
Per design decision D12, this bypasses ``dreamerv3/main.py`` rather than
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        help=(
            ...
            "the Phase-3 checkpoint layout."
        ),
    )
    p.add_argument(
        "--behavior-mixture",
        default="none",
        choices=["none", "type_balanced"],
        dest="behavior_mixture",
        help=(
            "Training-only behavior policy mixture. 'type_balanced' replaces each "
            "env's action with probability eps(t) by a draw uniform over the game's "
            "exposed action types and then uniform within the type (control arm for "
            "the factored-head result; see arc3_wm/behavior_mixture.py). Eval untouched."
        ),
    )
    p.add_argument("--mixture-eps0", type=float, default=0.3, dest="mixture_eps0",
                   help="Initial redraw probability for --behavior-mixture (linearly annealed to 0).")
    p.add_argument("--mixture-anneal-steps", type=int, default=200_000, dest="mixture_anneal_steps",
                   help="Env steps over which the mixture probability anneals to 0.")
    return p
...
DEFAULT_ARC3_ENV = {
    "max_steps": 1000,
    "use_seed": True,
    # Training-only state-change bonus (arc3_wm/state_change_reward.py).
    # 0.0 = off (the paper's native-reward protocol). Applied to the
    # training env factory only; eval envs always see native rewards.
    "shaping_beta": 0.0,
    "shaping_gate": "novelty",
    # Fresh game on every reset (episodes always start at level 1). Off by
    # default so training matches the paper's runs; the frozen-checkpoint
    # evaluation passes --env.arc3.full_reset True. See ARC3GymEnv.__init__.
    "full_reset": False,
}
...
def seed_wm_from_ckpt(agent: Any, ckpt_path: Path) -> dict[str, Any]:
    ...
    if matched_keys != WM_KEY_COUNT:
        raise ValueError(
            ...
            f"See the Phase-3 checkpoint layout."
        )
...
# ----------------------------------------------------------------------
# Behavior-mixture helpers (training-only control arm; laptop-testable)
# ----------------------------------------------------------------------

def exposed_types_for_game(game_id: str, seed: int = 0) -> list[int]:
    """Action types (1..7) the engine exposes at reset, from the Gym env's mask."""
    from arc3_wm.behavior_mixture import exposed_types_from_mask
    from arc3_wm.env import ARC3GymEnv

    env = ARC3GymEnv(game_id=game_id, seed=seed)
    try:
        _, info = env.reset(seed=seed)
        return exposed_types_from_mask(info["action_mask"])
    finally:
        env.close()


def wrap_agent_with_mixture(agent, mixture: str, types, eps0: float, anneal_steps: int, seed: int):
    """Return ``agent`` unchanged for mixture='none', else the mixture proxy."""
    if mixture == "none":
        return agent
    if mixture != "type_balanced":
        raise ValueError(f"unknown behavior mixture {mixture!r}")
    from arc3_wm.behavior_mixture import TypeBalancedMixturePolicy

    return TypeBalancedMixturePolicy(agent, types, eps0=eps0, anneal_steps=anneal_steps, seed=seed)
...
def maybe_shape_env(env, arc3_cfg, shaped: bool):
    """Wrap ``env`` in StateChangeRewardWrapper iff this is a training env
    (``shaped=True``) and ``arc3_cfg['shaping_beta'] > 0``. Eval envs and
    the beta=0 default return ``env`` unchanged."""
    beta = float(arc3_cfg.get("shaping_beta", 0.0) or 0.0)
    if not shaped or beta <= 0.0:
        return env
    from arc3_wm.state_change_reward import StateChangeRewardWrapper

    return StateChangeRewardWrapper(env, beta=beta, gate=str(arc3_cfg.get("shaping_gate", "novelty")))


def make_env(config, index: int, shaped: bool = False, **overrides):
    """Build an embodied.Env. arc3_<game> tasks dispatch to ARC3EmbodiedEnv.

    ``shaped=True`` marks a *training* env: the state-change bonus (if
    ``env.arc3.shaping_beta > 0``) is applied below the dtype/space
    wrappers. Eval factories never pass it.
    """
    if str(config.task).startswith("arc3_"):
        ...
        full_reset = bool(arc3_cfg.get("full_reset", False))
        env = ARC3EmbodiedEnv(game_id=game_id, seed=seed, max_steps=max_steps, full_reset=full_reset)
        env = maybe_shape_env(env, arc3_cfg, shaped)
...
def eval_sink_path_for(logdir) -> Path:
    """Where EvalRewardSink writes for a run: ``{logdir}/eval_episodes.jsonl``."""
    return Path(logdir) / "eval_episodes.jsonl"


def make_sinked_env_factory(make_env_fn, sink_path):
    """Wrap an env factory so every env it builds records eval rewards.

    ``make_env_fn(cfg, index, **overrides)`` is called unchanged and its
    result is wrapped in ``arc3_wm.eval_reward_sink.EvalRewardSink``
    writing to ``sink_path``. Laptop-importable: the sink import is
    deferred to call time.
    """
    sink_path = Path(sink_path)

    def factory(cfg, index, **overrides):
        from arc3_wm.eval_reward_sink import EvalRewardSink

        env = make_env_fn(cfg, index, **overrides)
        return EvalRewardSink(env, sink_path=sink_path)
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    mixture_types: list[int] = []
    if args.behavior_mixture != "none":
        if not str(config.task).startswith("arc3_"):
            raise ValueError("--behavior-mixture requires an arc3_<game> task")
        mixture_types = exposed_types_for_game(config.task.split("_", 1)[1], seed=int(config.seed))
        print(f"Behavior mixture: {args.behavior_mixture} over types {mixture_types} "
              f"eps0={args.mixture_eps0} anneal_steps={args.mixture_anneal_steps}")
    arc3_cfg_for_print = config.env.get("arc3", {}) if str(config.task).startswith("arc3_") else {}
    if float(arc3_cfg_for_print.get("shaping_beta", 0.0) or 0.0) > 0.0:
        print(f"State-change bonus (training env only): beta={arc3_cfg_for_print['shaping_beta']} "
              f"gate={arc3_cfg_for_print.get('shaping_gate', 'novelty')}")
    ...
    def make_agent_with_seed():
        ...
        return wrap_agent_with_mixture(
            agent, args.behavior_mixture, mixture_types,
            eps0=args.mixture_eps0, anneal_steps=args.mixture_anneal_steps, seed=int(config.seed),
        )
    ...
    # Eval env factory wrapped with EvalRewardSink so each eval episode's
    # reward stream lands in {logdir}/eval_episodes.jsonl for
    # scripts/compute_rhae.py to consume post-hoc. Training rollouts use
    # the unwrapped factory - wrapping them would record policy-noise
    # rewards (no RHAE signal) and balloon the file. Append mode survives
    # preemption-resume; compute_rhae's MIN-across-episodes aggregation
    # tolerates duplicated entries. Shared by train_eval (periodic evals)
    # and eval_only (frozen-checkpoint evaluation at a chosen N).
    eval_sink_path = eval_sink_path_for(config.logdir)
    make_env_eval_with_sink = make_sinked_env_factory(make_env, eval_sink_path)

    if config.script == "train":
        embodied.run.train(
            make_agent_with_seed,
            bind(make_replay, config, "replay"),
            bind(make_env, config, shaped=True),
        ...
        # Frozen-checkpoint evaluation: --run.from_checkpoint <ckpt folder>,
        # --run.steps sized to the episode count wanted. Every eval episode
        # is captured by the sink exactly as in train_eval.
