...
from arc3_wm.official_rhae import compute_official_rhae  # noqa: E402
...
def format_summary(
    *, game_id: str, step: Optional[int], metrics: Mapping
    ...
    official_key = f"eval/rhae_official/per_game/{game_id}"
    corpus_key = f"eval/rhae/per_game/{game_id}"
    levels = int(
        metrics.get(
            f"eval/rhae/levels_completed/{game_id}",
            metrics.get(f"eval/rhae_official/levels_completed/{game_id}", 0),
        )
    )
    parts = [f"levels_completed={levels}"]
    # ``per_game_rhae`` keeps the corpus value under its original name and
    # format: the Phase-4 shell harnesses parse this line.
    if corpus_key in metrics:
        parts.append(f"per_game_rhae={float(metrics[corpus_key]):.2f}")
    if official_key in metrics:
        parts.append(f"official_rhae={float(metrics[official_key]):.4f}")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        "--print-metrics",
        action="store_true",
        help="Also print the full metric family to stdout.",
    )
    parser.add_argument(
        "--metric",
        choices=["official", "corpus", "both"],
        default="both",
        help=(
            "Which metric to report. 'official' reproduces the toolkit scorer "
            "(shipped baselines, all levels in the denominator, game cap) and is "
            "the number of record; 'corpus' is the replay-derived D-A/D-B variant "
            "used through Phase 4; 'both' prints them side by side."
        ),
    )
    parser.add_argument(
        "--environment-files",
        ...
        help="Where cached game metadata.json files live (official metric only).",
    )
    ...
    if args.metric in ("official", "both"):
        metrics.update(
