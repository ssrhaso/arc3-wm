...
# discipline, same NaN/OOM detection as the patched proper harness.
#
# Run from the repo root on the Vast A100 box. Substrate smoke must pass first.
#
# Usage:
#   bash scripts/launch_phase4_fromscratch.sh 2>&1 | tee /workspace/logdir/p4-fromscratch-harness.log
#
...
export WANDB_NOTES="Phase 4 from-scratch (cold, no warm-start), stock DV3, sha=${SHA7}"
...
run_one() {
  ...
  export WANDB_TAGS="phase-4,arm:cold,game:${game},seed:${seed},from-scratch,substrate:a100"
...
echo ""
echo "============= PHASE 4 FROM-SCRATCH COMPLETE ============="
